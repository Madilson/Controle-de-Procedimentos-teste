...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="p-4 bg-gray-800/90 dark:bg-gray-900/90 border border-gray-600 rounded-lg shadow-xl text-white z-50">
  ...
  return (
    <div className="space-y-8 my-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-4">Análise por Região</h2>
            <div className="w-full h-72 md:h-96">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <XAxis dataKey="region" tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" />
                        <YAxis tickFormatter={formatCurrencyAxis} tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" width={60} />
                        <Tooltip
                           content={<CustomTooltip />}
                           cursor={{fill: 'rgba(128, 128, 128, 0.1)'}}
                        />
                        <Legend wrapperStyle={{ fontSize: '12px' }} />
                        <Bar name="Realizado" dataKey="valuePerformed" fill="#22c55e" radius={[4, 4, 0, 0]} />
                        <Bar name="Faturado" dataKey="valueBilled" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                        <Bar name="Pago" dataKey="valuePaid" fill="#a855f7" radius={[4, 4, 0, 0]} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-4">Tendência de Valor Pago</h2>
            <div className="w-full h-72 md:h-96">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                        data={lineChartData}
                    ...
                    >
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(128, 128, 128, 0.2)" />
                        <XAxis dataKey="date" tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" />
                        <YAxis tickFormatter={formatCurrencyAxis} tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" width={60} />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 md:p-6">
            <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-4">Valor Pago por Mês</h2>
            <div className="w-full h-72 md:h-96">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                    ...
                    >
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(128, 128, 128, 0.2)" />
                        <XAxis dataKey="month" tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" />
                        <YAxis tickFormatter={formatCurrencyAxis} tick={{ fill: '#6b7280', fontSize: 12 }} stroke="#6b7280" width={60} />
                        <Tooltip
                           content={<CustomTooltip />}
                           cursor={{fill: 'rgba(128, 128, 128, 0.1)'}}
                        ...
                        <Bar name="Valor Pago" dataKey="valuePaid" fill="#a855f7" radius={[4, 4, 0, 0]}>
                             <LabelList dataKey="valuePaid" position="top" formatter={formatCurrencyAxis} style={{ fill: '#6b7280', fontSize: 10 }} />
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
            </div>
